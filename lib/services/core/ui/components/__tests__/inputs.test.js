...
describe("when only type and id are set", () => {
  ...
  it("creates an input with the id and type set", () => {
    expect(input.outerHTML).toBe(
      `<input id="input-id" type="text" autocomplete="off" style="${inputStyle}">`,
    );
  });

  it("creates a label for the input", () => {
    expect(label.outerHTML).toBe(`<label style="${labelStyle}" for="input-id"></label>`);
  });
...
describe("when type, id, required and autocomplete are set", () => {
  ...
  it("creates an input with the id and type set", () => {
    expect(input.outerHTML).toBe(
      `<input id="input-id" type="text" autocomplete="on" style="${inputStyle}" required="">`,
    );
  });
...
describe("when type, id, minlength, maxlength and pattern are set", () => {
  ...
  it("creates an input with the id and type set", () => {
    expect(input.outerHTML).toBe(
      `<input id="input-id" type="text" autocomplete="off" style="${inputStyle}" minlength="4" maxlength="8" pattern="^.{4,8}$">`,
    );
  });
