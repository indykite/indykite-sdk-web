...
jest.mock("node-fetch", () => jest.fn().mockResolvedValue({ json: jest.fn() }));
...
beforeEach(() => {
  ...
  nodeFetch.mockImplementation(() =>
    Promise.resolve({
      json: jest.fn().mockResolvedValue({ "@type": "fail", error: "Not implemented" }),
      status: 501,
    }),
  );
...
describe("when server returns a fail message type", () => {
  beforeEach(async () => {
    nodeFetch.mockImplementation(() =>
      Promise.resolve({
        json: jest.fn().mockResolvedValue({ "@type": "fail" }),
        status: 200,
      }),
    );
...
describe("when server returns a logical message type", () => {
  describe("when no custom data are passed", () => {
    beforeEach(async () => {
      nodeFetch.mockImplementation(() =>
        Promise.resolve({
          json: jest.fn().mockResolvedValue(examples.logicalRegistration),
          status: 200,
        }),
      );
    ...
    it("sends correct request", () => {
      ...
      expect(nodeFetch.mock.calls[0][0]).toBe(`${BASE_URI}/auth/v2/${APPLICATION_ID}`);
      expect(JSON.parse(nodeFetch.mock.calls[0][1].body)).toEqual({
      ...
      expect(nodeFetch.mock.calls[0][1].headers).toEqual({
        "ikui-action-name": "setup",
        "ikui-version": expect.any(String),
      });
      expect(nodeFetch.mock.calls[0][1].method).toBe("POST");
    });
