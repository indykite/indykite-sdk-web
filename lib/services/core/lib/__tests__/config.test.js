...
describe("getBaseAuthUrl", () => {
  describe("when a custom version is specified", () => {
    beforeEach(() => {
      window.IKSDK.config.version = "v1";
    });

    it("returns correct auth URL", () => {
      expect(getBaseAuthUrl()).toBe("http://www.example.com/auth/v1/12345");
    });
  });

  it("returns correct auth URL", () => {
    expect(getBaseAuthUrl()).toBe("http://www.example.com/auth/v2/12345");
  });
});
