...
const oidcActionIdPath = "@indykite/oidc-action-id";
...
/**
 * Save the action ID so that it can be used in the response
 * after returning from an external OIDC provider.
 * @param id {string | null}
 */
const setOidcActionId = (id) => {
  if (id === null) {
    localStorage.removeItem(oidcActionIdPath);
  }

  localStorage.setItem(oidcActionIdPath, id);
};

/**
 * Get the action ID so that it can be used in the response
 * after returning from an external OIDC provider.
 * @returns {(string | null)}
 */
const getOidcActionId = () => {
  return localStorage.getItem(oidcActionIdPath);
};
 ...
 * @param threadId
 */
const setThreadId = (threadId) => localStorage.setItem(threadIdPath, threadId);
const getThreadId = () => localStorage.getItem(threadIdPath);

/**
 * Uses Session Storage
 * @param threadId
 */
const setFPThreadId = (threadId) => localStorage.setItem(forgottenPasswordThreadIdPath, threadId);
const getFPThreadId = () => localStorage.getItem(forgottenPasswordThreadIdPath);
