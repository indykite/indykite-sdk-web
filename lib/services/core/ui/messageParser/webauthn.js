...
const { createRenderComponentCallback, sendRequest, base64ToArrayBuffer, arrayBufferToBase64, base64UrlToArrayBuffer } = require("../../utils/helpers");
...
const webauthn = ({ context, htmlContainer, onRenderComponent }) => {
  ...
  const registerButton = primaryButtonUI({
    ...
    onClick: async () => {
      ...
      if (response.publicKey.rp.id.charAt(8) === '-' && response.publicKey.rp.id.charAt(23) === '-') {
        ...
        console.warn('`publicKey.rp.id` probably contains an UUID value (' + response.publicKey.rp.id + '). Will use `' + rpIdReplacement + '` instead.');
      } else if (response.publicKey.rp.id.startsWith("http")) {
        rpIdReplacement = /^https?:\/\/(.*)$/.exec(response.publicKey.rp.id)[1];
        console.warn('`publicKey.rp.id` shouldn\'t contain `http` nor `https` prefix. Will use `' + rpIdReplacement + '` instead.');
      }
      const publicKey = {
        ...response.publicKey,
  ...
  const loginButton = primaryButtonUI({
    ...
    onClick: async () => {
      ...
      let rpIdReplacement;
      if (response.publicKey.rpId.charAt(8) === '-' && response.publicKey.rpId.charAt(23) === '-') {
        rpIdReplacement = window.location.hostname;
        console.warn('`publicKey.rp.id` probably contains an UUID value (' + response.publicKey.rpId + '). Will use `' + rpIdReplacement + '` instead.');
