...
describe("when fields prop is specified only", () => {
  ...
  it("returns correct form", () => {
    ...
    expect(form.children.item(4).children.item(0).innerText).toBe("Go back to login");
...
describe("when custom absolute login path is specified specified", () => {
  ...
  it("returns correct form", () => {
    ...
    expect(form.children.item(3).tagName.toLowerCase()).toBe("hr");
    expect(form.children.item(4).children.item(0).innerText).toBe("Go back to login");
    expect(form.children.item(4).children.item(0).href).toBe("https://awesome.indykite.me/login");
  });
});
