 ...
 * @param {Parameters<typeof renderForm>[0]} props
...
const refreshUiScreen = (props, renderElement, response) => {
  ...
  if (typeof props.onBeforeRender === "function") {
    const newChild = props.onBeforeRender(child);
    if (newChild) {
      child = newChild;
    }
  }
 ...
 * @param {string} [props.labels.setNewPasswordButton]
 ...
 * @param {function} [props.onBeforeRender] Callback function which is called with a prepared form which is going to be rendered
...
const renderForm = (props) => {
  ...
  return setupRequest(
    Object.assign({ otpToken: props.otpToken }, window.IKSDK.config, {
