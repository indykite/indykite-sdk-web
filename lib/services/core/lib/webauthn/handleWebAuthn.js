...
const handleWebAuthn = async ({ id, publicKey, onFailCallback, onSuccessCallback }) => {
  try {
    let response;
    if (Utils.isAssertWebauthnRequest(publicKey)) {
      response = await handleAssertCredentials(publicKey, id);
    } else {
      response = await handleCreateCredentials(publicKey, id);
    }
