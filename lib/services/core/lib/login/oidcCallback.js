...
const oidcCallback = async () => {
  ...
  try {
    ...
    if (parsedParams.params) {
      try {
        const params = Base64.parse(parsedParams.params);
        const decodedParams = JSON.parse(Utf8.stringify(params));
        data.params = decodedParams;
      } catch (err) {
        console.error("Corrupted params");
        console.error(err);
      }
    }
